...
- Sharing should open a first-party share sheet before optional native share, generate a referral-tagged report URL behind the scenes, and present that flow as a polished result-sharing surface rather than a raw utility dialog. The sheet should keep the visible hierarchy tight: reward summary and report preview first, no exposed long URL block, then a stable action area with save-card first and copy/native share beneath it.
