...
- Production infra is now largely configured: Supabase + `pgvector`, Upstash Redis, Fly app `contractguard-prod`, Vercel frontend at `https://contractguard.jp`, backend API at `https://api.contractguard.jp`, and Sentry-backed observability are all in place, and both frontend/backend `/api/health` checks now return 200 in the production path.
- Production mail routing is now also split by responsibility: Google Workspace receives human replies at `support@contractguard.jp`, while Resend sends system emails from `noreply@mail.contractguard.jp` with `Reply-To: support@contractguard.jp`.
...
backend/
  ...
  config.py       # pydantic-settings (OPENAI_API_KEY, DATABASE_URL, REDIS_URL, KOMOJU, payment-method file path, etc.)
  ...
  services/
    ...
    email.py          # Resend API client (9-language subjects; configurable From + Reply-To)
...
- Sharing should open a first-party share sheet before optional native share, generate a referral-tagged report URL behind the scenes, and present that flow as a polished result-sharing surface rather than a raw utility dialog. The sheet should keep the visible hierarchy tight: a small kicker plus real title/description header, a large near-full-width preview card, no exposed long URL block, then a stable action area with save-card first and copy/native share beneath it.
