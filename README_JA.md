...
- 決済状態には二段構えの保護を追加しました。KOMOJU checkout を途中で閉じても、フロントエンドは 5 分後に無限ポーリングを抜けて「支払い未確認」状態へ遷移し、バックエンド側も `payment.failed` / `payment.cancelled` / `payment.expired` webhook を終端ステータスへ反映します
