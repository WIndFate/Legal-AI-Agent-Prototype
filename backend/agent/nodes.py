...
NON_CONTRACT_ERROR_CODE = "non_contract_document"
...
class NonContractDocumentError(Exception):
    def __init__(self, message: str = "Uploaded content does not appear to be a contract") -> None:
        super().__init__(message)
        self.code = NON_CONTRACT_ERROR_CODE
...
def parse_contract(state: AgentState) -> dict:
    ...
    messages = [
        ...
        HumanMessage(content=f"""以下の文書が日本語の契約書かどうかを先に判定してください。
...
{{
  ...
  "rejection_reason": "契約書ではない場合の理由。契約書なら空文字",
  "clauses": [
...
判定ルール:
- 契約書・合意書・利用規約・覚書・申込契約など、当事者間の権利義務や条件を定める文書なら `is_contract=true`
- 単なるメール、案内文、ニュース記事、説明文、履歴書、メモ、請求書だけの文書なら `is_contract=false`
- 契約書らしいが条項番号がなくても、義務・条件・禁止事項・解除・報酬などの契約構造が明確なら `is_contract=true`
...
{state["contract_text"]}"""),
    ...
    if not is_contract:
        raise NonContractDocumentError(rejection_reason or "Uploaded content does not appear to be a contract")
